...
- Create a choropleth map to visualize the number of stores per state, and another for population by state. Add custom hover text. Use `make_subplots` to place the maps side by side.
...
fig = make_subplots(
    ...
    subplot_titles=["Starbucks Stores per State", "Population per State"]
)

fig.add_trace(map1, row=1, col=1)
fig.add_trace(map2, row=1, col=2)

fig.update_geos(scope="usa", projection_type="albers usa", showlakes=True)
